namespace Project.Shared
{
    class Events
    {
        public const string PLAYER_CONNECT = "PLAYER_CONNECT";

        public const string UI_LOADED = "UI_LOADED";

        public const string UI_LOGIN_SHOW = "UI_LOGIN_SHOW";
        public const string UI_LOGIN_SUBMIT = "UI_LOGIN_SUBMIT";
        public const string UI_LOGIN_ERROR = "UI_LOGIN_ERROR";

        public const string UI_REGISTER_SHOW = "UI_REGISTER_SHOW";
        public const string UI_REGISTER_SUBMIT = "UI_REGISTER_SUBMIT";
        public const string UI_REGISTER_ERROR = "UI_REGISTER_ERROR";
    }
}
